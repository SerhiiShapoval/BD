...
app.get('/ads/:adId/comments', async (req, res) => {
  try {
...
app.get('/ads/:adId/tags', async (req, res) => {
  try {
    const adId = req.params.adId;
    const ad = await Ad.findByPk(adId);
    if (!ad) {
  ...
  try {
    ...
    const ad = await Ad.findOne({
      ...
      include: [
        { model: Comment },
        { model: Tag },
        { model: User }, // Включення моделі User для отримання автора оголошення
        { model: Category } // Включення моделі Category для отримання категорії оголошення
      ]
    });
...
app.get('/order-eager/:orderId', async (req, res) => {
  try {
    const adId = req.params.orderId;
    const { include } = req.query; // Отримання параметра include з запиту

    const includeModels = [];

    // Перевірка наявності параметра include у запиті
    if (include) {
      const models = include.split(','); // Розділення параметра include по комі
      models.forEach(model => {
        // Додавання моделей до списку includeModels
        switch (model.trim().toLowerCase()) {
          case 'user':
            includeModels.push({ model: User });
            break;
          case 'comments':
            includeModels.push({ model: Comment });
            break;
          case 'tags':
            includeModels.push({ model: Tag });
            break;
          case 'category':
            includeModels.push({ model: Category });
            break;
          default:
            break;
        }
    ...
    // Отримання оголошення з включеними відповідними моделями
    const ad = await Ad.findOne({
      where: { id: adId },
